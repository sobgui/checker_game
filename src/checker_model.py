...
class CheckerModel:
        ...
        for row, col in product(range(0, ROWS), range(0, COLS)):
            if type(self.checker_grid[row][col]) == Piece:
                ...
                if current_piece.player == self.turn:
                    depth, all_moves_for_current_piece = self.get_possible_moves_for_current_piece(current_piece,
                                                                                                   depth=0,
                                                                                                   all_moves_for_current_piece=[])
        ...
        for row_to_check, col_to_check in cells_to_check:
            ...
            for row_to_verify in range(row, row_to_check, 1 if row_to_check > row else -1):
                for col_to_verify in range(col, col_to_check, 1 if col_to_check > col else -1):
                    if type(self.checker_grid[row_to_verify][
                                col_to_verify]) == Piece and row_to_verify != row and col_to_verify != col:
                        stop_verify = True
            ...
            if stop_verify:
                ...
                # déplacement simple
                if cell_state == "accessible" and depth == 0 and (
                        row_to_check - row != self.turn or current_piece.king):
                    ...
                    if self.get_cell_state(row_arrival, col_arrival) == "accessible":
                        ...
                        if depth == 0:
                            ...
                            if depth <= all_moves_for_current_piece[-1].get_depth():
                                current_move_object = all_moves_for_current_piece[-1].extract_common_deplacement(
    ...
    def check_game_state(self):
        ...
        if len(self.history) >= 15:
            draw_game = True
            game_evaluation = self.evaluate_grid(self.history[-25:])
            for checker_grid in self.history[-24:]:
                if self.evaluate_grid(checker_grid) != game_evaluation:
                    draw_game = False
